...
)
{
    ...
    double desiredTwist[3] = { 0.0, 0.0, -0.02 };
    ...
    // ABAG parameters
    const double alphaPosition[3] = { 0.9, 0.9, 0.95 };
    ...
    while (totalElapsedTime < TASK_TIME_LIMIT_MICRO)
    {
        ...
        if (jnt_torque(5) > -0.4) {
            std::cout << "torques: joint 2: "  << jnt_torque(1) <<  ", joint 4: " << jnt_torque(3)
                  << ", joint 6: " << jnt_torque(5) << std::endl;
            break;
        }
        ...
        if (iterationCount == 1)
        {
            ...
            desiredPositionLog << desiredTwist[0] << "," << desiredTwist[1] << "," << desiredTwist[2] << std::endl;
            if (desiredPositionLog.is_open()) desiredPositionLog.close();
        }
        ...
        errorPos[0] = desiredTwist[0] - endEffTwist.p.v(0);
        errorPos[1] = desiredTwist[1] - endEffTwist.p.v(1);
        errorPos[2] = desiredTwist[2] - endEffTwist.p.v(2);
        ...
        // control
        kc::writeDataRow(logPosX, abagStatePos[0], totalElapsedTime.count(), errorPos[0], commandPos[0], endEffTwist.p.v(0));
        kc::writeDataRow(logPosY, abagStatePos[1], totalElapsedTime.count(), errorPos[1], commandPos[1], endEffTwist.p.v(1));
        kc::writeDataRow(logPosZ, abagStatePos[2], totalElapsedTime.count(), errorPos[2], commandPos[2], endEffTwist.p.v(2));
